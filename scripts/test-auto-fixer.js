...
class TestAutoFixer {
  ...
  log(message, type = 'info') {
    const icons = { info: '🧪', success: '✅', error: '❌', warning: '⚠️', snapshot: '📸' };
            ...
            for (const [alias, actualPath] of Object.entries(importMap)) {
              if (line.includes(alias)) {
                ...
                if (newLine !== line) {
                  lines[i] = newLine;
                  hasChanges = true;
                  this.log(`Fixed import in ${testFile}: ${alias} -> ${relativePath}`, 'success');
                }
              }
            }
          }
  ...
  fixCommonTestPatterns(testFiles) {
    for (const testFile of testFiles) {
      try {
        ...
        if (content.includes('toBeInTheDocument') || content.includes('toHaveClass')) {
          if (!content.includes('@testing-library/jest-dom') && !content.includes('jest-dom')) {
            ...
            if (lastImportIndex >= 0) {
              imports.splice(lastImportIndex + 1, 0, "import '@testing-library/jest-dom';");
        ...
        this.log(`Failed to fix patterns in ${testFile}: ${e.message}`, 'error');
      }
    }
  }
  ...
  generateMissingTests() {
    ...
    for (const componentDir of componentDirs) {
      ...
      const componentFile = fs.readdirSync(componentPath)
...
`;
  ...
  async run() {
    ...
    if (finalResult.success) {
      ...
      this.log('⚠️ Some test issues remain (may require manual review)', 'warning');
      console.log('\nTest output:');
